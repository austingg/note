...
class BaiduTieBaSpider(scrapy.spiders.Spider):
    name = 'baidutieba'
    # start_urls = ['http://tieba.baidu.com/p/2235516502?see_lz=1&pn=%d' % i for i in range(1, 18)]
    start_urls = ['http://image.baidu.com/search/index?tn=baiduimage&word=花朵&pn=%d' % i for i in range(1, 18)]
    image_names = {}

    def parse(self, response):
        item = BaiDuTieBaItem()
        item['image_urls'] = response.xpath("//li[@data-objurl]").extract()
